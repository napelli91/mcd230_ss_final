...
|:---:|:---:|:---:|:---:|:---:|
|`r as.character(vg.fit$model)` | 0 | `r vg.fit$psill` | `r vg.fit$range` | `r vg.fit$kappa` |
|`r as.character(vg.best.fit$model)` | 0 | `r vg.best.fit$psill` | `r vg.best.fit$range` | `r vg.best.fit$kappa` |
...
dens.plot.2 <- ggplot(polys_sf) +
    ...
    ggtitle("Varianza de valores predichos [Exp]") +
    coord_sf(datum=st_crs(4326)) +
    scale_fill_viridis(option = "magma", direction = 1)
...
Los estadísticos obtenidos de esta cross validación se resumen en la sigueinte tabla
