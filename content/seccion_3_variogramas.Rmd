...
En función de todo lo analizado y observado gráficamente, decidimos proseguir ajustando el variograma teórico al modelo con tendencia lineal en las coordenadas. 

### Variograma teórico

Para elegir el modelo teórico que mejor ajuste a nuestros datos utilizamos la función *fit.variogram*. En primer lugar, cabe aclarar que como ésta función admite como *input* variogramas construídos con la función *variogram*, nuestro variograma empírico a ajustar es el siguiente:
...
Por otro lado, con ayuda de dicha función exploramos varias distribuciones teóricas y obtuvimos, para cada una, los valores óptimos de los parámetros. Finalmente, para poder elegir observamos visualmente el variograma en conjunto con las distintas distribuciones teóricas y calculamos la suma de cuadrado del error para cada opción. Los resultados se resumen en la siguiente tabla: 
...
El modelo teórico que mejor se ajuste a nuestro modelo empírico será aquel que tenga menor error SSErr. En este caso, podemos elegir entre el *Mattern*, *Bessel* o el *Pentaspherical*. 
...
Elegimos quedarnos con el modelo teórico Mattern para ajustar nuestro variograma empírico y procedemos utilizarlo para realizar las predicciones.
