...
Luego de haber analizado la autocorrelación de nuestros datos, pasamos a pensar si el proceso que los generó es estacionario o no. Con ayuda de un *plot* podemos rápidamente observar si hay tendencia tanto en x como en y, y la normalidad de nuestro proceso:

```{r trend}
d = sp_data_final # guardamos el set de datos recortado en un objeto
coordinates(d) = ~longitude+latitude
...
Aún modelando la tendencia en y vemos en el variograma mapa que el proceso no se asimila a uno isotrópico. Nos preguntamos si se debía quizás a una tendencia en x que no estábamos observando, pero al modelar con ambas tendencias de forma lineal los resultados no cambian.

Finalmente, resolvimos trabajar de manera direccional. 
...
En función de todo lo analizado y observado gráficamente, decidimos ajustar el variograma teórico al variograma direccional  con dirección $\frac{3\pi}{4}$. Esta elección se sustenta no solo en elegir la dirección que más simétrica parece en el variograma mapa sin tendencia, sino también sobre el hecho de que la dirección de 135° es la que más estable parece.
