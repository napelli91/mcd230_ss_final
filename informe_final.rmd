...
analizar el comportamiento de la temperatura promedio en tierra durante la primavera. Para
esto, debemos entender la estructura del archivo GDX que contiene los datos. 

> Estructura del archivo GDX
```
...
Year1Season3 Gridpoint1 Gridpoint2 Gridpoint3 Gridpoint4 .....Gridpointy
...
Cada fila en el dataset tiene 9100 puntos representando una grilla de 70x130 llenandose
de izquierda a derecha y de arriba hacia abajo. Esto quiere decir, el primer punto 
corresponde a las coordenadas (-25,70), luego (-24.5,70), y así sucesivamente.

Estos datos primeramente segmentados los vamos a convertir en un archivo *csv* que luego
podamos ingresar en R para su posterior procesamiento.

A partir de este archivo csv, filtramos algunos
valores: datos con valor -999,99 y _NaN's_. Luego de ello tomamos el área correspondiente
...
entre sí. Por otro lado, si miramos el los gráficos de dispersión del eje y, se puede ver claramente la
tendencia en la variable latitud. En cuanto al eje x (longitud), no vemos una tendencia distinguible 
a siemple vista. Finalmente, vemos la distribución del proceso transformado pareciera asemejarse a una normal.
...
Con este variograma nube vemos que a partir de un **h=8** se comienza a ver la meseta, corroborando nuestras conclusiones de la sección anterior.
